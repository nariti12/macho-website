...
2. 男性向けの定番ブランドは楽天 Item Search API で補完候補を少数追加する
3. 商品名から内容量、たんぱく質量、女性向け/美容系キーワードを正規表現ベースで抽出
4. `SAVAS / ザバス`、シェイカー、バー、子供向けなどの除外ルールを適用
5. `my-best` の男性向け / 女性向け記事に掲載された商品を補助加点に変換する
6. 男性向け / 女性向けのスコアを計算
7. Supabase の `products` / `product_metrics` / `rankings` に保存
6. `/supplements-top3` は保存済みデータを表示

男性向けは楽天順位を主軸に、レビュー、たんぱく質情報、価格妥当性、定番ブランド、`my-best` 掲載を補助にして再選抜します。女性向けは楽天順位を主軸に、レビュー、ソイ適性、女性向けキーワード、`my-best` 掲載を補助にして再選抜します。
